const testButton = document.getElementById("deleteRoom");
const chatInput = document.getElementById("chat-box");
const usernameInput = document.getElementById("username-input");
const dialog = document.getElementById("dialog");
const addNewRoomBtnContainer = document.getElementById(
  "add-new-room-btn-container"
);
...
ws.onmessage = (event) => {
  ...
  if (data.type == "deleteRoom") {
    const prevActiveRoom = getActiveRoom();
    roomsDiv.innerHTML = "";
    const roomnames = [];
...
usernameInput.addEventListener("keydown", (e) => {
  if (e.key == "Enter") {
    ...
    ws.send(
      JSON.stringify({ type: "usernameInput", username: usernameInput.value })
    );
  }
});
...
function deleteRoom(room) {
  ws.send(JSON.stringify({ type: "deleteRoom", room: room }));
}
