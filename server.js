...
function writeChatHistory(data) {
  ...
  writeFile("./data/ChatHistory.json", parsedData, "utf8", function(err) {
    if (err) {
      console.error(err);
    }
...
wss.on("connection", (ws) => {
  ...
  ws.send(
    JSON.stringify({ type: "clientList", clients: Array.from(clientList) })
  );
  ...
  ws.on("message", function message(data) {
    ...
    if (dataMessage.type == "chatMessage") {
      chatHistory[dataMessage.room].messages.push({
    ...
    if (dataMessage.type == "addNewRoom") {
      ...
      wss.clients.forEach(function each(client) {
        if (client.readyState === WebSocket.OPEN) {
          client.send(
            JSON.stringify({ type: "addNewRoom", room: dataMessage.room })
          );
        }
      });
    }
    ...
    if (dataMessage.type == "deleteRoom") {
      console.log("test", chatHistory);
      delete chatHistory[dataMessage.room];
      console.log("what", dataMessage, chatHistory);
      wss.clients.forEach(function each(client) {
        if (client.readyState == WebSocket.OPEN) {
          client.send(
            JSON.stringify({ type: "deleteRoom", history: chatHistory })
          );
        }
      });
    }
